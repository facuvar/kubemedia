...
function initializeAdPreview() {
    loadCustomerAds();
    updateAdPreview();
}

function loadCustomerAds() {
    const select = document.getElementById('previewAdSelect');
    if (!select) return;
    
    // Clear existing options except the first one
    select.innerHTML = '<option value="">Elegí un anuncio para previsualizar</option>';
    
    // Get customer ads from localStorage
    const customerAds = JSON.parse(localStorage.getItem('customerAds') || '[]');
    
    // Sample ads for demo
    const sampleAds = [
        {
            id: 'demo_1',
            title: 'Pizza 2x1 - Viernes de ofertas',
            description: 'Todos los viernes pizza 2x1 en nuestra sucursal del centro',
            category: 'food',
            status: 'active',
            mediaType: 'image',
        ...
        },
        {
            ...
            description: 'Un mensaje especial para la persona más importante',
            category: 'personal',
            status: 'pending',
            mediaType: 'image',
        ...
        {
            id: 'demo_3',
            title: 'Descuento especial Black Friday',
            description: '50% off en toda la tienda este fin de semana',
            category: 'retail',
            status: 'active',
            mediaType: 'image',
            mediaUrl: '../calendario.png',
            views: 8750,
            duration: '30 seg',
            engagement: '6.8%'
        }
    ...
    // Combine real ads with sample ads
    const allAds = [...customerAds, ...sampleAds];
    
    allAds.forEach(ad => {
        const option = document.createElement('option');
        option.value = ad.id;
        option.textContent = `${ad.title} (${ad.status === 'active' ? 'Activo' : ad.status === 'pending' ? 'Pendiente' : 'Pausado'})`;
...
function updateAdPreview() {
    const select = document.getElementById('previewAdSelect');
    const selectedAdId = select?.value;
    ...
    if (selectedAd) {
        displayAdPreview(selectedAd);
        displayAdInfo(selectedAd);
        displayAdStats(selectedAd);
    }
}
    ...
    if (screenContent) {
        screenContent.innerHTML = `
            <div class="preview-placeholder">
                ...
                <h3>Pantalla digital</h3>
                <p>Seleccioná un anuncio para ver cómo se verá en pantalla</p>
            </div>
        `;
    }
    ...
    if (previewInfo) {
        previewInfo.innerHTML = `
            <div class="info-placeholder">
                ...
                <p>Los detalles del anuncio seleccionado aparecerán aquí</p>
            </div>
        `;
    }
    
    // Reset stats
    const statsElements = ['previewViews', 'previewDuration', 'previewEngagement'];
...
function displayAdPreview(ad) {
    ...
    if (ad.mediaType === 'image' && ad.mediaUrl) {
        screenContent.innerHTML = `
            <img src="${ad.mediaUrl}" alt="${ad.title}" class="preview-media" />
        `;
    } else if (ad.mediaType === 'video' && ad.mediaUrl) {
        screenContent.innerHTML = `
            <video src="${ad.mediaUrl}" class="preview-media" autoplay muted loop></video>
        `;
    } else {
        // Text-only ad
        screenContent.innerHTML = `
            <div style="padding: 2rem; text-align: center; color: white; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); width: 100%; height: 100%; display: flex; flex-direction: column; justify-content: center;">
                <h2 style="font-size: 2rem; margin-bottom: 1rem; font-weight: bold;">${ad.title}</h2>
                <p style="font-size: 1.2rem; opacity: 0.9;">${ad.description}</p>
            </div>
        `;
    }
}
    ...
    previewInfo.innerHTML = `
        <div class="ad-info">
            <div class="info-item">
                ...
                <span class="info-value">${ad.title}</span>
            </div>
            <div class="info-item">
                <span class="info-label">Descripción</span>
                <span class="info-value">${ad.description || 'Sin descripción'}</span>
            </div>
            <div class="info-item">
                <span class="info-label">Categoría</span>
                <span class="info-value">${categoryLabels[ad.category] || ad.category}</span>
            </div>
            <div class="info-item">
                <span class="info-label">Estado</span>
                <span class="status-badge ${ad.status}">${statusLabels[ad.status] || ad.status}</span>
            </div>
            <div class="info-item">
                <span class="info-label">Tipo de medio</span>
                <span class="info-value">${ad.mediaType === 'image' ? 'Imagen' : ad.mediaType === 'video' ? 'Video' : 'Texto'}</span>
            </div>
        </div>
    `;
}

function displayAdStats(ad) {
    const viewsElement = document.getElementById('previewViews');
    const durationElement = document.getElementById('previewDuration');
    const engagementElement = document.getElementById('previewEngagement');
    
    if (viewsElement) {
        viewsElement.textContent = ad.views > 0 ? ad.views.toLocaleString() : '--';
    }
    
    if (durationElement) {
        durationElement.textContent = ad.duration || '--';
    }
    
    if (engagementElement) {
        engagementElement.textContent = ad.engagement || '--';
    }
}

function refreshPreview() {
    const refreshBtn = document.querySelector('.btn-refresh-preview');
    if (refreshBtn) {
        ...
        setTimeout(() => {
            ...
            showNotification('Preview actualizado correctamente', 'success');
        }, 1000);
    }
}

// Initialize ad preview when page loads
document.addEventListener('DOMContentLoaded', function() {
    // ... existing code ...
    initializeAdPreview();
}); 